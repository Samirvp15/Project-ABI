    ...
    <div className="w-full p-4">
      ...
      {(filterSummary || dateSummary) && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {filterSummary && (
            <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[10px] font-medium text-amber-800 dark:text-amber-300">
              {filterSummary}
            </span>
          )}
          {dateSummary && (
            <span className="rounded-full border border-sky-500/30 bg-sky-500/10 px-2 py-0.5 text-[10px] font-medium text-sky-800 dark:text-sky-300">
              {dateSummary}
            </span>
          )}
        </div>
...
export function ChatInlineChart({ chart }: ChatInlineChartProps) {
  ...
  const { xLabel, yLabel } = getChartAxisLabels(chart as DashboardWidget);
  const filterSummary = chart.config.filter_summary;
  const dateSummary = chart.config.date_filter_summary;
  const angledXTicks = type === "bar" || type === "histogram" || type === "line";
  const margin = chartMarginsWithLabels(!!xLabel, !!yLabel, { angledXTicks });
